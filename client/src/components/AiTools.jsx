    ...
    <div className='px-4 sm:px-20 xl:px-32'>
      ...
      <div className='flex flex-wrap mt-10 justify-center'>
        {AiToolsData.map((tool, index) => (
          <div
            key={index}
            className='p-8 m-4 max-w-xs rounded-xl bg-[#FDFDFE] shadow-lg border border-gray-100 hover:translate-y-1 transition-all cursor-pointer'
            onClick={() => {
              if (isSignedIn) {
                navigate(tool.path);
              } else {
                alert("Sign In to your account, click on Get Started"); 
              }
            }}
          >
            <tool.Icon
